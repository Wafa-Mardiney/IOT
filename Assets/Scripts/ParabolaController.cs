...
public class ParabolaController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ParabolaRoot = SceneManager.Instance.allRoots[Random.Range(0, SceneManager.Instance.allRoots.Length)];
    }
    ...
    void Update()
    {
        ...
        if (!Animation && SceneManager.Instance.sendingSignals)
        {
            GetComponent<AraTrail>().enabled = false;
            FollowParabola();
        }
    ...
    public void FollowParabola()
    {
        ...
        Animation = true;
        //GetComponent<AraTrail>().enabled = true;
